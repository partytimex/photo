var main = (function($) {
    var _ = {
        ...
        initProperties: function() {
            ...
            if (skel.vars.IEVersion < 9) _.$window.on('resize', function() {
        ...
        initEvents: function() {
            ...
            _.$viewer.on('touchstart', function(event) {
                ...
                if (_.$viewer.touchPosX === null || _.$viewer.touchPosY === null) return;
                // Calculate stuff.
                ...
                // Swipe left (next).
                if ((diffY < boundary && diffY > (-1 * boundary)) && (diffX > delta)) _.next();
                // Swipe right (previous).
                else if ((diffY < boundary && diffY > (-1 * boundary)) && (diffX < (-1 * delta))) _.previous();
                // Overscroll fix.
            ...
            _.$main.on('touchstart', function(event) {
                // Bail on xsmall.
                if (skel.breakpoint('xsmall').active) return;
                // Record start position.
        ...
        switchTo: function(index, noHide) {
            ...
            if (_.current == index && !skel.breakpoint('xsmall').active) return;
            // Locked? Bail.
            if (_.locked) return;
            // Lock.
            _.locked = true;
            // Hide main wrapper if medium is active.
            if (!noHide && skel.breakpoint('medium').active && skel.vars.IEVersion > 8) _.hide();
            // Get slides.
