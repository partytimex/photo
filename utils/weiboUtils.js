...
module.exports = {
                        ...
                        if (rs.length > 0) {
                            ...
                            if (!d.title || !d.attribute || !d.description || !d.thumbnail_pic) {
                                bingUtils.fetchStory({
                                    ...
                                    d['url'] = !!d.qiniu_url && 'http://api.1ptime.com/bing/' + d.qiniu_url + '_1920x1080.jpg' || d.url;
                                    d['title'] = body.title;
    ...
    commonSend: function(data, callback, isAuto) {
        module.exports.checkOauth(function(token) {
            ...
            var full = y + '-' + m + '-' + d;
            var status = ('#1ptoday# ' + full + ' / #' + data.title + '# ' + data.description).slice(0, 130) + '... http://1ptoday.com?id=' + data.id;
            var post = {
                access_token: token,
                status: status,
                url: data.url,
